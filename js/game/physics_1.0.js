...
/*--
zim.Physics = function(frame, borders, gravity, scroll)
...
makeBorders({x, y, width, height}) default is the frame bounds - pass in "none" for no borders
	...
	zim.Physics = function(frame, borders, gravity, scroll) {
		...
		this.Drag = function(list) {
			...
			var mousedownEvent = stage.on("stagemousedown", function() {
				isMouseDown = true;
				mouseX = (stage.mouseX-(scroll?stage.x:0))/scale;
				mouseY = (stage.mouseY-(scroll?stage.y:0))/scale;
		...
		this.makeBorders = function(rect) {
